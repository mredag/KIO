...
│   ├── /webhook/whatsapp     — Meta webhook → forwards to n8n
│   └── /webhook/instagram    — Meta webhook → OpenClaw (USE_OPENCLAW=true) or n8n
├── OpenClaw (port 18789) — AI agent gateway (replaces n8n for Instagram)
│   └── Instagram hook        — Kimi K2 via OpenRouter, multi-model routing
├── n8n (Pi only, port 5678) — Workflow automation (WhatsApp only now)
...
**Seed:** `backend/src/database/seed-knowledge.ts` (idempotent, 61 Turkish entries — migrated 2026-02-23)
...
Meta Webhook POST → KIO /webhook/instagram
  → InstagramContextService.analyzeMessage(senderId, text)
    → getConversationHistory (last 10 messages from DB)
    → detectIntent (Turkish keyword → category mapping)
    → classifyModelTier (light/standard/advanced)
  → Fetch FILTERED knowledge + customer data (parallel)
    → ?categories=services,pricing (from intent detection)
  → Build enriched Turkish prompt (includes conversation history)
  → POST to OpenClaw /hooks/instagram (202, hook mapping routes to main agent)
    → messageTemplate: "{{message}}" passes enriched prompt through
    → sessionKey: "hook:instagram:{{senderId}}"
  → Poll ~/.openclaw/agents/main/sessions/ for response (45s timeout, 2s interval)
    → Session key: agent:main:hook:instagram:{senderId}
    → Looks for stopReason=stop, falls back to stable assistant text
  → **Policy Agent validation** (ResponsePolicyService — Gemini Flash Lite)
    ...
    → If FAIL: direct OpenRouter correction (~2-3s) instead of re-dispatching to OpenClaw (~15-20s)
  ...
  → Log via /api/integrations/instagram/interaction (model_used, tokens_estimated)
  → MC Integration (fire-and-forget):
    → UPSERT mc_conversations (channel=instagram)
    → INSERT mc_cost_ledger (job_source=instagram, agent_id=instagram)
