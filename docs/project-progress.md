...
- 2026-03-16: Kiosk Chromium now defaults back to hardware rendering under the managed Wayland boot path, with `KIOSK_GPU_MODE=software` kept only as an explicit fallback. Survey mode background effects were also reduced so the Pi spends less time on decorative animation during active surveys.
