...
interface NavItem { id: string; label: string; icon: ReactNode; path: string; badge?: number | string; }
interface NavGroup { id: string; label: string; items: NavItem[]; }
interface SidebarProps { isCollapsed: boolean; onToggleCollapse: () => void; isMobileOpen: boolean; onCloseMobile: () => void; }
...
export default function Sidebar({ isCollapsed, onToggleCollapse, isMobileOpen, onCloseMobile }: SidebarProps) {
  ...
  const [expanded, setExpanded] = useState<Record<string, boolean>>({
    command: true, operations: true, channels: true, content: true, business: true, system: false,
  });
  ...
  const groups: NavGroup[] = [
    { id: 'command', label: 'KOMUTA MERKEZİ', items: [
      ...
      { id: 'mc-activity', label: 'Canlı Akış', path: '/admin/mc/activity', icon: I('M5.636 18.364a9 9 0 010-12.728m12.728 0a9 9 0 010 12.728m-9.9-2.829a5 5 0 010-7.07m7.072 0a5 5 0 010 7.07M13 12a1 1 0 11-2 0 1 1 0 012 0z') },
      ...
      { id: 'mc-cron', label: 'Zamanlayıcı', path: '/admin/mc/cron', icon: I('M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z') },
    ]},
    { id: 'operations', label: 'OPERASYONLAR', items: [
      ...
      { id: 'workflow-test', label: 'DM Simülatör', path: '/admin/workflow-test', icon: I('M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664zM21 12a9 9 0 11-18 0 9 9 0 0118 0z') },
      { id: 'mc-costs', label: 'API Kullanımı', path: '/admin/mc/costs', icon: I('M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z') },
    ...
    { id: 'channels', label: 'KANALLAR', items: [
      { id: 'mc-conversations', label: 'Konuşmalar', path: '/admin/mc/conversations', icon: I('M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z') },
      ...
      { id: 'interactions', label: 'Etkileşimler', path: '/admin/interactions', icon: I('M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z') },
    ]},
    { id: 'content', label: 'İÇERİK', items: [
      { id: 'knowledge-base', label: 'Bilgi Bankası', path: '/admin/knowledge-base', icon: I('M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253') },
    ...
    { id: 'business', label: 'İŞLETME', items: [
  ...
  const renderItem = (item: NavItem) => {
    ...
    return (
      <PrefetchLink key={item.id} to={item.path} onClick={onCloseMobile}
        ...
        title={isCollapsed ? item.label : undefined} aria-current={active ? 'page' : undefined}>
        ...
        {!isCollapsed && (
          <>
            ...
            {item.badge && <span className="px-1.5 py-0.5 text-[10px] font-semibold rounded-full bg-sky-500/15 text-sky-400 border border-sky-500/20">{item.badge}</span>}
          </>
        )}
      </PrefetchLink>
    );
    ...
    <div className="flex flex-col h-full">
      ...
      <nav className="flex-1 overflow-y-auto px-2 py-3 space-y-4 sidebar-scroll">
        {groups.map(g => {
          ...
          return (
            <div key={g.id}>
              {!isCollapsed && (
                <button onClick={() => toggle(g.id)} className="w-full flex items-center justify-between px-3 mb-1">
                  <span className="mc-label text-[9px]">{g.label}</span>
              ...
              {(isCollapsed || open) && <div className="space-y-0.5">{g.items.map(renderItem)}</div>}
            </div>
          );
        })}
      </nav>
      ...
      <div className="p-2 space-y-0.5">
        <button onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}
          ...
          {theme === 'dark' ? I('M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z') : I('M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z')}
          {!isCollapsed && <span className="text-[13px] font-medium">{theme === 'dark' ? t('theme.light') : t('theme.dark')}</span>}
        </button>
        <button onClick={onToggleCollapse}
          ...
          aria-label={isCollapsed ? 'Expand sidebar' : 'Collapse sidebar'} aria-expanded={!isCollapsed}>
          {isCollapsed ? I('M13 5l7 7-7 7M5 5l7 7-7 7') : I('M11 19l-7-7 7-7m8 14l-7-7 7-7')}
          {!isCollapsed && <span className="text-[13px] font-medium">{t('sidebar.collapse')}</span>}
        </button>
      </div>
    </div>
  );
  ...
  return (
    <>
      ...
      {isMobileOpen && <div className="md:hidden fixed inset-0 bg-black/60 backdrop-blur-sm z-40" onClick={onCloseMobile} />}
