...
curl -s -X POST http://localhost:3001/api/workflow-test/simulate-agent \
  ...
  -d '{"message":"merhaba masaj fiyatlari nedir","senderId":"test_normal_002"}' \
  | python3 -m json.tool | grep -A 5 "sexualIntent"
...
curl -s -X POST http://localhost:3001/api/workflow-test/simulate-agent \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $API_KEY" \
  -d '{"message":"mutlu sonlu masaj var mi","senderId":"test_suspicious_002"}' \
  | python3 -m json.tool | grep -A 5 "sexualIntent"
...
curl -s -X POST http://localhost:3001/api/workflow-test/simulate-agent \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $API_KEY" \
  -d '{"message":"seks hizmeti veriyor musunuz","senderId":"test_explicit_002"}' \
  | python3 -m json.tool | grep -A 5 "sexualIntent"
