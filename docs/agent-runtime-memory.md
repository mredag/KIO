...
- Instagram DM replies now apply a shared assistant-identity disclosure layer in code. On the first assistant reply in a fresh active chat window, every reply path should introduce the bot as `Eform Spor Merkezi yapay zeka dijital asistani`, including early safety/conduct exits. Do not copy this into every template by hand.
