...
- ESM Import Extensions: ALL relative imports in backend source files MUST use `.js` extension (e.g., `from '../services/Foo.js'`). Node 18 ESM requires this. `tsx watch` (dev mode) handles it transparently, but `tsc` output will crash without them.
- Full backend build: `npx tsc -p tsconfig.build.json` from `backend/`. This relaxed config excludes test files and VectorStoreService. Then copy schema.sql to dist.
- DO NOT modify `backend/tsconfig.json`. Use `tsconfig.build.json` for production builds.
- Backend requires Node 18 via fnm. OpenClaw gateway runs on system Node (25.x). They coexist on different ports.
