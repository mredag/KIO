...
- [ ] 25. Implement Kiosk Theme System





  - [x] 25.1 Create kiosk theme configuration and hook


    - Create `frontend/src/lib/kioskTheme.ts` with theme configurations (classic, neo, immersive)
    - Define KioskThemeConfig interface with background, text, button, qrContainer, progressBar, animations
    - Create `useKioskTheme` hook that reads from kioskStore and returns theme classes
    - Add coupon-specific theme overrides for emerald/teal color scheme
    - _Requirements: 15.1, 15.4_
  - [ ]* 25.2 Write property test for kiosk theme persistence
    - **Property 21: Kiosk Theme Persistence**
    - **Validates: Requirements 15.2, 15.3**
  - [x] 25.3 Refactor GoogleQRMode with theme support


    - Update `frontend/src/components/kiosk/GoogleQRMode.tsx`
    - Import and use `useKioskTheme` hook
    - Apply theme classes to container, title, subtitle, close button, QR container
    - Add theme-specific animations (float, glow effects)
    - Maintain existing functionality (close button, loading/error states)
    - _Requirements: 16.1, 16.2, 16.3, 16.4, 16.5_
  - [ ]* 25.4 Write property test for theme class application
    - **Property 22: Theme Class Application**
    - **Validates: Requirements 15.1, 15.4**
  - [x] 25.5 Refactor CouponQRMode with theme support


    - Update `frontend/src/components/kiosk/CouponQRMode.tsx`
    - Import and use `useKioskTheme` hook with 'couponQr' variant
    - Apply theme classes to container, countdown timer, QR container, progress bar
    - Add theme-specific animations and glow effects
    - Maintain existing functionality (countdown, auto-return to menu)
    - _Requirements: 17.1, 17.2, 17.3, 17.4, 17.5_
  - [x] 25.6 Add kiosk theme selector to Settings page


    - Update `frontend/src/pages/admin/SettingsPage.tsx` Kiosk Theme tab
    - Add visual theme selector with preview cards for each theme
    - Show theme name, description, and color preview
    - Save theme selection to backend settings
    --_Requirements: 15.2_

  - [x] 25.7 Write property test for theme transition
...
- [ ] 26. Checkpoint - Ensure kiosk theming works



  - Ensure all tests pass, ask the user if questions arise.
  - Test theme switching in admin settings
  - Verify Google QR mode displays correctly with each theme
  - Verify Coupon QR mode displays correctly with each theme
  - Test on Raspberry Pi kiosk display
-

- [ ] 27. Final Checkpoint - Complete integration




  - Ensure all tests pass, ask the user if questions arise.
  - Verify all admin dashboard pages work correctly
  - Verify all kiosk modes work correctly with theming
